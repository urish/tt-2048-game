...
async def test_project(dut):
    ...
    for r1, r0, g1, g0, b1, b0 in itertools.product(range(2), repeat=6):
        ...
        green = 170 * g1 + 85 * g0
        blue = 170 * b1 + 85 * b0
        palette[b0 << 6 | g0 << 5 | r0 << 4 | b1 << 2 | g1 << 1 | r1 << 0] = bytes(
            (red, green, blue)
        )
    ...
    async def check_line(expected_vsync):
        for i in range(H_TOTAL):
            ...
            assert hsync == (
                0 if H_SYNC_START <= i < H_SYNC_END else 1
        ...
        for i in range(H_TOTAL):
            ...
            if i < H_DISPLAY:
                framebuffer[offset + 3 * i : offset + 3 * i + 3] = palette[
                    dut.uo_out.value.integer & 0b01110111
                ]
            await ClockCycles(dut.clk, 1)
    ...
    for i in range(CAPTURE_FRAMES):
        if i == 3:
            dut.ui_in.value = 0x80  # Enable debug mode, should draw a rect on the frame
